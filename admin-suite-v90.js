...
var GROUPS=[
 ...
 ['Users & Payments',['users','payments','paymentreqs','course-enrollments','subscriptions']],
...
/* ---------- Automatic approved-course revenue ---------- */
...
async function syncCourseRevenue(id,quiet){
 var c=db();if(!c||!id)return false;try{
   var er=await c.from('course_enrollments').select('*').eq('id',id).single();if(er.error||!er.data)return false;var row=er.data,approved=row.payment_status==='approved'||row.enrollment_status==='enrolled';if(!approved)return false;
   var stamp=row.reviewed_at||row.access_granted_at||row.updated_at||row.created_at||new Date().toISOString(),parts=localDateParts(stamp),ref='COURSE:'+row.id,gross=n(row.price)||200,pct=await courseSharePct(),deduction=gross*(pct/100),net=Math.max(0,gross-deduction);
   ...
   var existing=await c.from('company_finance_entries').select('id').eq('payment_reference',ref).limit(1);if(existing.error){if(!quiet)console.warn('[V90 course revenue]',existing.error.message);return false}
   var payload={entry_type:'income',category:'Total Courses Revenue',subcategory:row.course_name||'Advanced Forex Course',amount:net,gross_amount:gross,share_deduction_amount:deduction,entry_date:parts.date,period_month:parts.month,note:'Auto Course Revenue — '+(row.course_name||'Advanced Forex Course')+' — '+(row.full_name||row.email||'Student'),payment_status:'paid',payment_method:row.payment_method||'Course Payment',payment_date:parts.date,payment_reference:ref,updated_at:new Date().toISOString()};
   ...
   if(out.error){if(!quiet)console.warn('[V90 course revenue]',out.error.message);return false}if(!quiet)toast('Course payment added to Company Revenue.','ok');return true;
 }catch(e){if(!quiet)console.warn('[V90 course revenue]',e);return false}
}
window.v90SyncCourseRevenue=syncCourseRevenue;
async function reconcileSelectedMonth(quiet){if(V90.courseSyncBusy)return;var c=db();if(!c)return;V90.courseSyncBusy=true;try{var monthEl=q('#crMonth'),month=monthEl&&monthEl.value?monthEl.value:(new Date().getFullYear()+'-'+String(new Date().getMonth()+1).padStart(2,'0'));var r=await c.from('course_enrollments').select('*').order('created_at',{ascending:false});if(r.error)return;var approved=(r.data||[]).filter(function(x){if(!(x.payment_status==='approved'||x.enrollment_status==='enrolled'))return false;var p=localDateParts(x.reviewed_at||x.access_granted_at||x.updated_at||x.created_at);return p.month.indexOf(month)===0});for(var i=0;i<approved.length;i++)await syncCourseRevenue(approved[i].id,true);if(!quiet&&approved.length)toast(approved.length+' approved course payment(s) synced.','ok')}finally{V90.courseSyncBusy=false}}
