...
/* ---------- Automatic approved PAID-course revenue (V91 hotfix) ---------- */
...
function isPaidCourseEnrollment(row){return !!row&&(String(row.course_type||'').toLowerCase()==='paid'||String(row.course_key||'').toLowerCase()==='advanced')}
...
async function syncCourseRevenue(id,quiet){
 var c=db();if(!c||!id)return false;try{
   ...
   var existing=await c.from('company_finance_entries').select('id,period_month').eq('payment_reference',ref).limit(1);if(existing.error){if(!quiet)console.warn('[V91 course revenue]',existing.error.message);return false}
   ...
   if(out.error){if(!quiet)console.warn('[V91 course revenue]',out.error.message);return false}if(!quiet)toast('Approved paid-course revenue added to Company Revenue.','ok');return true;
 }catch(e){if(!quiet)console.warn('[V91 course revenue]',e);return false}
}
...
async function cleanupInvalidAutoCourseRevenue(c,month,enrollments){
 ...
 var byId={};(enrollments||[]).forEach(function(x){byId[String(x.id)]=x});var bad=(fr.data||[]).filter(function(e){var ref=String(e.payment_reference||''),id=ref.slice(7),row=byId[id];return row&&!isApprovedPaidCourseEnrollment(row)}).map(function(e){return e.id});if(!bad.length)return 0;
 var del=await c.from('company_finance_entries').delete().in('id',bad);if(del.error){console.warn('[V91 course revenue cleanup]',del.error.message);return 0}return bad.length;
}
