(function(){
  'use strict';
  if(window.__pspLinkManagerV202)return;
  window.__pspLinkManagerV202=true;
  ...
  function bindUi(){
    ...
    document.getElementById('lmRefreshBtn').onclick=()=>Promise.all([loadLinks(),loadTeamMembersV202()]);
  ...
  async function loadTeamMembersV202(){
  ...
  function installShowPageHook(){
    ...
    window.showPage=function(page,el){
      ...
      if(page==='linkmanager'){
        ...
        setTimeout(()=>{loadTeamMembersV202();loadLinks();},0);
      }
      return result;
    };
  }
  ...
  function init(){addStyles();addMenuAndPage();installShowPageHook();setTimeout(()=>{loadTeamMembersV202();if(document.getElementById('page-linkmanager')?.classList.contains('active'))loadLinks();},150);setTimeout(setupRealtime,1000);}
