...
window.loadAdminUsers=async function(){
  ...
  var results=await Promise.all([
    ...
    db.from('course_enrollments').select('user_id,whatsapp,created_at').order('created_at',{ascending:false}).limit(1000)
  ]);
  var r=results[0],enr=results[1];if(r.error){console.error(r.error);return}
  var phoneByUser={};
  if(!enr.error)(enr.data||[]).forEach(function(row){if(row.user_id&&!phoneByUser[row.user_id]&&String(row.whatsapp||'').trim())phoneByUser[row.user_id]=String(row.whatsapp).trim()});
  allAdminUsers=(r.data||[]).map(function(user){
    var current=user.whatsapp||user.whatsapp_number||user.phone||user.mobile||'';
    if(!String(current||'').trim()&&phoneByUser[user.id])user.whatsapp=phoneByUser[user.id];
    return user;
  });
  window.adminUsers=allAdminUsers.slice();
  renderAdminUsersFinal();
};
...
window.loadAprList=async function(){
  ...
  var courseHtml=cv.length?cv.map(function(x){var st=x.payment_status||x.enrollment_status||'pending',provider=String(x.provider_status||'').trim().toLowerCase(),shown=provider||st;var label=shown;if(['accepted','success','successful','completed','complete','paid','approved','captured','confirmed','verified','settled'].includes(shown))label='Success';else if(['failed','failure'].includes(shown))label='Failed';else if(['rejected','declined'].includes(shown))label=shown==='declined'?'Declined':'Rejected';else if(shown==='expired')label='Expired';else if(['initiated','created','submitted'].includes(shown))label='Initiated';else if(['pending','processing','waiting','awaiting','in_process','queued'].includes(shown))label='Pending';var reason=String(x.provider_last_error||x.rejection_reason||'').trim();return '<div class="psp-pay-row"><div><strong>'+esc(x.full_name||'Member')+'</strong><br><small>'+esc(x.email||x.whatsapp||'')+'</small></div><div><strong>'+esc(x.course_name||'Advanced Forex Course')+'</strong><br><small>'+esc(x.payment_method||'Payment method not added')+'</small></div><div>$'+Number(x.price||200).toFixed(0)+'<br><small>'+esc(x.transaction_id||'No transaction ID')+'</small></div><div>'+pill(label)+(reason?'<br><small style="color:var(--text-muted)">'+esc(reason)+'</small>':'')+'</div><div>'+(x.receipt_url?'<a class="btn btn-secondary btn-sm" href="'+esc(x.receipt_url)+'" target="_blank">Receipt</a> ':'')+(st==='pending'?'<button class="btn btn-sm" onclick="approveCourseEnrollment(\''+x.id+'\')">Approve</button> <button class="btn btn-secondary btn-sm" onclick="rejectCourseEnrollment(\''+x.id+'\')">Reject</button>':'')+'</div></div>'}).join(''):'<div style="padding:18px;color:var(--text-muted)">No matching course payments.</div>';
