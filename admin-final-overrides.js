...
window.loadDashboardStats=async function(){
  ...
  try{
    var results=await Promise.all([
      ...
      db.from('mentors').select('*').catch?null:Promise.resolve(null)
    ].slice(0,2));
    ...
    setText('dashTotalMentors',mentorIds.size.toLocaleString());setText('dashPaidUsers',uniq(approved,'user_id').toLocaleString());setText('dashFreeUsers',uniq(free,'user_id').toLocaleString());setText('dashActiveCourses','2');setText('dashMonthlyRevenue','$'+revenue.toLocaleString());setText('dashPendingPayments',pending.length.toLocaleString());setText('dashApprovedEnrollments',approved.length.toLocaleString());
