(function(){
  ...
  function addStyles(){
    ...
    style.textContent=`
      ...
      #page-linkmanager{--lm-accent:#fb9201;--lm-navy:#0b1624;--lm-line:rgba(251,146,1,.24);padding-bottom:26px}
      #page-linkmanager>.lm-grid,#page-linkmanager>.lm-card{position:relative}
      .lm-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin:0 0 18px}
      .lm-stat{position:relative;overflow:hidden;min-height:124px;padding:18px 19px 16px;border:1px solid rgba(215,180,128,.46);border-radius:18px;background:linear-gradient(145deg,var(--bg-card) 0%,rgba(255,248,237,.92) 100%);box-shadow:0 15px 38px rgba(42,29,10,.08)}
      [data-theme="dark"] .lm-stat{background:linear-gradient(145deg,#111b2a,#0d1624);border-color:rgba(251,146,1,.22)}
      .lm-stat:after{content:'';position:absolute;right:-18px;top:-34px;width:112px;height:112px;border-radius:50%;background:radial-gradient(circle,rgba(251,146,1,.14),rgba(251,146,1,0) 70%);pointer-events:none}
      .lm-stat-head{display:flex;align-items:center;gap:8px;position:relative;z-index:1}
      .lm-stat-icon{width:32px;height:32px;display:grid;place-items:center;border-radius:10px;background:linear-gradient(145deg,#fff6e7,#ffe5be);border:1px solid rgba(251,146,1,.26);font-size:15px;box-shadow:0 7px 18px rgba(251,146,1,.12)}
      [data-theme="dark"] .lm-stat-icon{background:rgba(251,146,1,.12)}
      .lm-stat-head>span:not(.lm-stat-icon){font-size:9px;text-transform:uppercase;letter-spacing:.09em;color:var(--text-muted);font-weight:900}
      .lm-stat-head em{margin-left:auto;padding:4px 7px;border-radius:999px;background:rgba(251,146,1,.09);color:#c66c00;font-size:7px;font-style:normal;font-weight:900;text-transform:uppercase;letter-spacing:.06em}
      .lm-stat strong{display:block;position:relative;z-index:1;font-size:27px;line-height:1;margin:13px 0 7px;color:var(--text-primary);letter-spacing:-.03em}
      .lm-stat small{position:relative;z-index:1;font-size:9px;color:var(--text-muted);font-weight:650}
      .lm-card{overflow:hidden;margin-bottom:18px;border:1px solid rgba(215,180,128,.44)!important;border-radius:20px!important;background:linear-gradient(180deg,var(--bg-card),rgba(255,251,245,.84))!important;box-shadow:0 18px 44px rgba(42,29,10,.075)!important;padding:0!important}
      [data-theme="dark"] .lm-card{background:linear-gradient(180deg,#101a29,#0c1522)!important;border-color:rgba(251,146,1,.2)!important}
      .lm-card>.card-header{display:flex;align-items:center;justify-content:space-between;gap:14px;margin:0!important;padding:16px 18px!important;border-bottom:1px solid rgba(215,180,128,.35)!important;background:linear-gradient(90deg,rgba(251,146,1,.075),transparent 38%)}
      .lm-card-heading{display:flex;align-items:center;gap:11px}
      .lm-card-icon{width:38px;height:38px;display:grid;place-items:center;border-radius:12px;background:linear-gradient(145deg,#ff9f18,#e97f00);color:#fff;font-size:17px;box-shadow:0 9px 22px rgba(233,127,0,.22)}
      .lm-card .card-title{font-size:14px;font-weight:900;color:var(--text-primary);letter-spacing:-.01em}
      .lm-card .card-meta{margin-top:3px;font-size:9px;color:var(--text-muted);line-height:1.45}
      .lm-card-badge{display:inline-flex;align-items:center;gap:5px;padding:6px 9px;border:1px solid rgba(251,146,1,.23);border-radius:999px;background:rgba(251,146,1,.08);color:#c56b00;font-size:8px;font-weight:900;text-transform:uppercase;letter-spacing:.065em;white-space:nowrap}
      .lm-builder-body{padding:16px 18px 18px}
      .lm-form-grid{display:grid;grid-template-columns:1.2fr 1fr 1fr;gap:12px}
      .lm-form-grid .wide{grid-column:span 2}
      .lm-form-grid label{display:block;font-size:8.5px;color:var(--text-muted);font-weight:900;margin:0 0 6px;text-transform:uppercase;letter-spacing:.055em}
      .lm-form-grid input,.lm-form-grid select,.lm-form-grid textarea{width:100%;min-height:38px;padding:9px 11px;border:1px solid rgba(194,167,126,.52);border-radius:10px;background:rgba(255,252,247,.95);color:var(--text-primary);font:inherit;font-size:10px;outline:none;transition:border-color .18s ease,box-shadow .18s ease,background .18s ease}
      [data-theme="dark"] .lm-form-grid input,[data-theme="dark"] .lm-form-grid select,[data-theme="dark"] .lm-form-grid textarea{background:#0c1624;border-color:rgba(255,255,255,.12);color:#f8fafc}
      .lm-form-grid input:focus,.lm-form-grid select:focus,.lm-form-grid textarea:focus{border-color:var(--lm-accent);box-shadow:0 0 0 3px rgba(251,146,1,.09);background:var(--bg-card)}
      .lm-preview{display:flex;align-items:center;gap:11px;margin-top:15px;padding:10px 11px;border:1px solid rgba(251,146,1,.42);border-radius:13px;background:linear-gradient(90deg,rgba(251,146,1,.10),rgba(251,146,1,.025));word-break:break-all}
      .lm-preview-icon{width:34px;height:34px;display:grid;place-items:center;flex:0 0 auto;border-radius:10px;background:var(--lm-navy);color:#ff9a0a;font-size:16px}
      .lm-preview-copy{display:flex;min-width:0;flex:1;flex-direction:column;gap:3px}
      .lm-preview-copy small{font-size:7.5px;font-weight:900;letter-spacing:.08em;color:var(--text-muted)}
      .lm-link{font-size:10px;font-weight:850;color:#c76a00;white-space:normal;overflow-wrap:anywhere}
      .lm-create-btn{margin-left:auto!important;min-width:112px!important;padding:9px 13px!important;border-color:#e98200!important;background:linear-gradient(145deg,#ff9c14,#ea7e00)!important;color:#fff!important;box-shadow:0 8px 20px rgba(234,126,0,.22)!important}
      .lm-create-btn:hover{transform:translateY(-1px);filter:brightness(1.02)}
      .lm-status{display:inline-flex;padding:4px 8px;border-radius:999px;font-size:8px;font-weight:900;text-transform:uppercase;letter-spacing:.04em}.lm-status.on{background:var(--green-bg);color:var(--green)}.lm-status.off{background:var(--red-bg);color:var(--red)}
      .lm-actions{display:flex;gap:5px;flex-wrap:wrap}.lm-btn{border:1px solid rgba(194,167,126,.5);border-radius:8px;padding:6px 8px;background:var(--bg-card);color:var(--text-primary);cursor:pointer;font-size:8.5px;font-weight:850;transition:all .18s ease}.lm-btn:hover{border-color:var(--lm-accent);color:#c76a00;box-shadow:0 5px 14px rgba(251,146,1,.1)}
      .lm-clean-wrap{padding:14px 18px 18px}.lm-clean-links{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.lm-clean{position:relative;min-height:68px;padding:12px 76px 11px 12px;border:1px solid rgba(194,167,126,.42);border-radius:12px;background:linear-gradient(145deg,rgba(255,252,247,.92),rgba(246,238,224,.72));transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease}
      [data-theme="dark"] .lm-clean{background:#0d1725;border-color:rgba(255,255,255,.1)}
      .lm-clean:hover{transform:translateY(-1px);border-color:rgba(251,146,1,.42);box-shadow:0 8px 20px rgba(42,29,10,.06)}
      .lm-clean strong{display:block;margin-bottom:5px;font-size:10px;color:var(--text-primary)}.lm-clean code{display:block;font-size:8.5px;color:var(--text-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.lm-clean .lm-btn{position:absolute;right:10px;top:50%;transform:translateY(-50%);float:none!important}
      .lm-table-wrap{padding:0 14px 14px;overflow:auto}.lm-table-wrap .data-table{margin:0;border-collapse:separate;border-spacing:0 6px}.lm-table-wrap .data-table thead th{padding:9px 8px;border:0;background:transparent;color:var(--text-muted);font-size:7.5px;letter-spacing:.055em;text-transform:uppercase}.lm-table-wrap .data-table tbody tr{background:rgba(255,252,247,.82);box-shadow:0 3px 10px rgba(42,29,10,.025)}[data-theme="dark"] .lm-table-wrap .data-table tbody tr{background:#0d1724}.lm-table-wrap .data-table tbody td{padding:9px 8px;border-top:1px solid rgba(194,167,126,.28);border-bottom:1px solid rgba(194,167,126,.28);font-size:9px}.lm-table-wrap .data-table tbody td:first-child{border-left:1px solid rgba(194,167,126,.28);border-radius:10px 0 0 10px}.lm-table-wrap .data-table tbody td:last-child{border-right:1px solid rgba(194,167,126,.28);border-radius:0 10px 10px 0}
      .lm-refresh-btn{border-color:rgba(251,146,1,.3)!important;color:#b96600!important;background:rgba(251,146,1,.06)!important}
      .lm-modal{position:fixed;inset:0;z-index:5000;background:rgba(3,9,20,.65);display:none;place-items:center;padding:18px;backdrop-filter:blur(8px)}.lm-modal.open{display:grid}.lm-modal-card{width:min(850px,100%);max-height:84vh;overflow:auto;background:var(--bg-card);border:1px solid rgba(251,146,1,.28);border-radius:20px;box-shadow:0 30px 80px rgba(0,0,0,.35)}.lm-modal-head{display:flex;justify-content:space-between;align-items:center;padding:17px 19px;border-bottom:1px solid var(--border)}.lm-modal-body{padding:18px}.lm-event{display:grid;grid-template-columns:120px 130px 1fr;gap:12px;padding:10px 0;border-bottom:1px solid var(--border);font-size:10px}.lm-event b{text-transform:capitalize}.lm-empty{padding:28px;text-align:center;color:var(--text-muted)}
      .lm-wa-card{width:min(470px,100%);overflow:hidden;background:var(--bg-card);border:1px solid rgba(245,158,11,.32);border-radius:20px;box-shadow:0 28px 80px rgba(0,0,0,.34)}.lm-wa-top{padding:20px 20px 13px;background:linear-gradient(135deg,var(--gold-bg),transparent);border-bottom:1px solid var(--border)}.lm-wa-icon{width:42px;height:42px;border-radius:13px;display:grid;place-items:center;background:rgba(37,211,102,.12);border:1px solid rgba(37,211,102,.25);font-size:21px;margin-bottom:11px}.lm-wa-top h3{margin:0;color:var(--text-primary);font-size:18px}.lm-wa-top p{margin:6px 0 0;color:var(--text-muted);font-size:11px;line-height:1.55}.lm-wa-body{padding:18px 20px 20px}.lm-wa-body label{display:block;font-size:10px;font-weight:900;letter-spacing:.06em;text-transform:uppercase;color:var(--text-muted);margin-bottom:7px}.lm-wa-body input{width:100%;box-sizing:border-box;padding:12px 13px;border-radius:11px;border:1px solid var(--border);background:var(--bg-elevated);color:var(--text-primary);font:inherit;outline:none}.lm-wa-body input:focus{border-color:var(--lm-accent);box-shadow:0 0 0 3px rgba(251,146,1,.12)}.lm-wa-hint{margin-top:8px;color:var(--text-muted);font-size:10px;line-height:1.45}.lm-wa-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:17px}.lm-wa-actions .primary{background:var(--lm-accent);border-color:var(--lm-accent);color:#111827}.lm-wa-link-name{display:inline-flex;margin-top:9px;padding:5px 8px;border-radius:999px;background:var(--bg-elevated);border:1px solid var(--border);font-size:9px;font-weight:850;color:#c76a00}
      @media(max-width:900px){.lm-grid{grid-template-columns:1fr 1fr}.lm-form-grid{grid-template-columns:1fr}.lm-form-grid .wide{grid-column:auto}.lm-clean-links{grid-template-columns:1fr 1fr}.lm-event{grid-template-columns:90px 100px 1fr}.lm-table-wrap{padding-inline:10px}}
      @media(max-width:560px){.lm-grid,.lm-clean-links{grid-template-columns:1fr}.lm-actions{min-width:190px}.lm-event{grid-template-columns:1fr;gap:3px}.lm-card>.card-header{align-items:flex-start;flex-direction:column}.lm-card-badge{align-self:flex-start}.lm-preview{align-items:flex-start;flex-wrap:wrap}.lm-create-btn{width:100%;margin-left:0!important}.lm-clean{padding-right:72px}}
    `;
  ...
  function addMenuAndPage(){
    ...
    page.innerHTML=`
      <div class="lm-grid">
        <div class="lm-stat"><div class="lm-stat-head"><span class="lm-stat-icon">🔗</span><span>Active Links</span><em>Live</em></div><strong id="lmActive">0</strong><small>Links currently enabled</small></div>
        <div class="lm-stat"><div class="lm-stat-head"><span class="lm-stat-icon">↗</span><span>Total Clicks</span><em>Traffic</em></div><strong id="lmClicks">0</strong><small>All tracked link visits</small></div>
        <div class="lm-stat"><div class="lm-stat-head"><span class="lm-stat-icon">👤</span><span>Signups</span><em>Leads</em></div><strong id="lmSignups">0</strong><small>Registered users attributed</small></div>
        <div class="lm-stat"><div class="lm-stat-head"><span class="lm-stat-icon">✓</span><span>Enrollments</span><em>Result</em></div><strong id="lmEnrollments">0</strong><small>Course enrollment conversions</small></div>
      </div>
      <div class="card lm-card lm-builder-card">
        <div class="card-header"><div class="lm-card-heading"><div class="lm-card-icon">🔗</div><div><div class="card-title">Generate Tracked Link</div><div class="card-meta">Create a campaign link, assign a team reference and track every conversion.</div></div></div><span class="lm-card-badge">Growth Tool</span></div>
        <div class="lm-builder-body">
          ...
          <div class="lm-preview"><div class="lm-preview-icon">↗</div><div class="lm-preview-copy"><small>TRACKED URL PREVIEW</small><span class="lm-link" id="lmPreview">${BASE_DOMAIN}/free-course?ref=person-1</span></div><button class="lm-btn lm-create-btn" type="button" id="lmCreateBtn">Create Link</button></div>
        </div>
      </div>
      <div class="card lm-card">
        <div class="card-header"><div class="lm-card-heading"><div class="lm-card-icon">⌁</div><div><div class="card-title">Clean Page URLs</div><div class="card-meta">Quick-copy official PipSePaisa pages without campaign tracking.</div></div></div><span class="lm-card-badge">Direct Links</span></div>
        <div class="lm-clean-wrap"><div class="lm-clean-links" id="lmCleanLinks"></div></div>
      </div>
      <div class="card lm-card">
        <div class="card-header"><div class="lm-card-heading"><div class="lm-card-icon">📊</div><div><div class="card-title">Tracked Links Performance</div><div class="card-meta">Clicks, unique visitors and conversions update automatically.</div></div></div><button class="btn btn-secondary btn-sm lm-refresh-btn" id="lmRefreshBtn">↻ Refresh</button></div>
        <div class="table-wrap lm-table-wrap"><table class="data-table"><thead><tr><th>Link</th><th>Source</th><th>Destination</th><th>Clicks</th><th>Unique</th><th>Signups</th><th>Enrollments</th><th>Conversion</th><th>Status</th><th>Actions</th></tr></thead><tbody id="lmTable"><tr><td colspan="10">Open Link Manager to load data.</td></tr></tbody></table></div>
      </div>
  ...
  function renderCleanLinks(){
    ...
    box.innerHTML=pages.map(([name,path])=>`<div class="lm-clean"><strong>${esc(name)}</strong><code>${BASE_DOMAIN}${esc(path)}</code><button class="lm-btn" onclick="copyLinkV42('${BASE_DOMAIN}${path}')">Copy</button></div>`).join('');
  }
