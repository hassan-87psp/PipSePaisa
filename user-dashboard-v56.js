...
function activityHTML(signals,charts,articles){
  ...
  return '<div class="psp58-activity">'+a.slice(0,6).map(x=>`<div class="psp58-act"><div class="psp58-act-icon ${x.cls}">${x.ico}</div><div><b>${esc(x.t)}</b><span>${esc(x.m)}</span></div><time>${esc(fmtPKT(x.d))} PKT<br><small>${esc(relative(x.d))}</small></time></div>`).join('')+'</div>';
}
...
async function load(force=false){
  ...
  try{
    ...
    root.innerHTML=`<div class="psp58-home">
      ...
      <section class="psp58-grid main"><div class="psp58-card featured"><div class="psp58-card-head"><div><span class="eyebrow">MARKET SIGNAL</span><h3>Latest Signal</h3></div><button class="psp58-link" onclick="psp58Nav('signals')">View All →</button></div>${signalHTML(signals[0],allowed)}</div><div class="psp58-card class-card"><div class="psp58-card-head"><div><span class="eyebrow">LIVE LEARNING</span><h3>Next Live Class</h3></div><button class="psp58-link" onclick="navMyCoursesV58()">My Courses →</button></div>${nextClassHTML(classes,enrollments,courses)}</div></section>
      <section class="psp58-grid lower"><div class="psp58-card"><div class="psp58-card-head"><div><span class="eyebrow">MARKET INTELLIGENCE</span><h3>Latest Market Analysis</h3></div><button class="psp58-link" onclick="psp58Nav('articles')">Explore →</button></div>${analysisHTML(charts,articles,allowed)}</div><div class="psp58-card"><div class="psp58-card-head"><div><span class="eyebrow">YOUR TRADING JOURNAL</span><h3>Journal Snapshot</h3></div><button class="psp58-link" onclick="psp58Nav('journal')">Open Journal →</button></div>${journalHTML(trades,allowed)}<div class="psp58-tools"><button onclick="psp58Nav('newshub')">📡 <span>World News Hub</span></button><button onclick="psp58Nav('strength')">💪 <span>Strength Meter</span></button><button onclick="psp58Nav('charts')">📊 <span>Live Charts</span></button><button onclick="psp58Nav('aireport')">🤖 <span>AI Report</span></button></div></div></section>
      <section class="psp58-card activity-card"><div class="psp58-card-head"><div><span class="eyebrow">LIVE FEED</span><h3>Recent Activity</h3></div><button class="psp58-link" onclick="PSPUserDashboard.load(true)">↻ Refresh</button></div>${activityHTML(signals,charts,articles)}</section>
    </div>`;
