(function(){
  ...
  function upgradeTools(){
    ...
    if(grid&&grid.children.length===5){
      ...
      card.innerHTML='<span class="num">06</span><div class="iconbox">'+SVG.chart+'</div><h3>Economic Calendar</h3><p>Review scheduled economic events and plan your analysis around periods of potential market volatility.</p><a href="./?page=calendar">View Calendar →</a>';
  ...
  function upgradeToolCardBanners(){
    ...
    cards.slice(0,banners.length).forEach((card,index)=>{
      ...
      banner.href=existingLink?.getAttribute('href')||'./';
  ...
  function fixLinks(){
    document.querySelectorAll('a').forEach(a=>{
      if(a.matches('.topbar .login'))a.href='/sign-in';
      ...
      else if(a.closest('.broker-grid,.broker-logo-grid,.partner-promo-grid'))a.href='/partner#partner-programs';
  ...
  function improveSEO(){
    const map={
      ...
      'partner':['Partner Education & Support | PipSePaisa','Explore PipSePaisa partner education, team training, marketing resources and broker-program information.'],
      ...
      'trading-tools':['Trading Tools & Services | PipSePaisa','Use practical trading journals, calculators, market resources and educational services from PipSePaisa.'],
