...
function inject(){
 ...
 .mc-market{margin-top:18px}.mc-market-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:12px}.mc-market-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}
 .mc-shop{border:1px solid var(--border);border-radius:16px;background:var(--bg-card);padding:18px;position:relative;overflow:hidden}.mc-shop:after{content:'🎓';position:absolute;right:16px;top:14px;font-size:48px;opacity:.1}
 .mc-shop h3{margin:0 0 6px;font-size:18px}.mc-price{font-size:24px;font-weight:900;color:var(--gold);margin:10px 0}.mc-shop p{font-size:11px;color:var(--text-muted);line-height:1.55;max-width:80%}
 @media(max-width:700px){.mc-market-grid{grid-template-columns:1fr}}
 ...
 const content=document.getElementById('content');if(content&&!document.getElementById('page-mycourses')){const p=document.createElement('div');p.className='page';p.id='page-mycourses';p.innerHTML='<div class="card" style="margin-bottom:14px"><div class="card-header" style="margin-bottom:0"><div><div class="card-title">🎓 My Courses</div><div class="card-meta" style="margin-top:4px">Your enrolled courses and learning progress</div></div></div></div><div id="myCoursesGrid" class="mc-pro-grid"></div><section class="mc-market"><div class="mc-market-head"><div><div class="card-title">Explore Courses</div><div class="card-meta">Enroll directly from your User Panel</div></div></div><div class="mc-market-grid"><article class="mc-shop"><h3>Basic Forex Course</h3><div class="mc-price">100% Free</div><p>Build your foundation with structured beginner lessons, technical analysis and risk-management basics.</p><button class="btn" type="button" onclick="openCourseEnrollment(\'basic\')">Start Free Course</button></article><article class="mc-shop"><h3>Advanced Forex Course</h3><div class="mc-price">$200</div><p>Professional trading education with advanced market structure, fundamentals and strategy development.</p><button class="btn" type="button" onclick="openCourseEnrollment(\'advanced\')">Enroll Now</button></article></div></section>';content.appendChild(p);}
}
function card(r){
