...
function append(panel,node,wide){if(!panel||!node)return;if(wide)node.classList.add('psp-v124-wide');panel.appendChild(node)}
function buildEditorTabs(){
  ...
  if(document.getElementById('pspV124CourseTabs')){activateTab('basic');return;}
  ...
  const tabs=document.createElement('div');tabs.id='pspV124CourseTabs';tabs.className='psp-v124-editor-tabs';
  const defs=[['basic','Basic Info'],['access','Pricing & Access'],['page','Page Content'],['learning','Learning'],['modules','Modules']];
  tabs.innerHTML=defs.map(([k,l])=>`<button type="button" class="psp-v124-editor-tab" data-tab="${k}">${l}</button>`).join('');
  const panels=document.createElement('div');panels.className='psp-v124-editor-panels';
  defs.forEach(([k])=>{const p=document.createElement('div');p.className='psp-v124-editor-panel '+k;p.dataset.panel=k;panels.appendChild(p)});
  ...
  const switchWrap=pub?.closest('div');if(switchWrap&&switchWrap.contains(prem)){switchWrap.classList.add('psp-v124-switches');p('access').appendChild(switchWrap)}
  ...
  const footer=document.getElementById('saveCourseBtn')?.parentElement;if(footer)footer.classList.add('psp-v124-course-footer');
  tabs.addEventListener('click',e=>{const b=e.target.closest('[data-tab]');if(b)activateTab(b.dataset.tab)});
  activateTab('basic');
}
function activateTab(key){
  document.querySelectorAll('#modal-courseForm .psp-v124-editor-tab').forEach(b=>b.classList.toggle('active',b.dataset.tab===key));
  document.querySelectorAll('#modal-courseForm .psp-v124-editor-panel').forEach(p=>p.classList.toggle('active',p.dataset.panel===key));
}
...
function init(){
  ...
  if(page&&window.MutationObserver){new MutationObserver((mutations)=>{
    const external=mutations.some(m=>{
      ...
      return !(n&&n.closest&&n.closest('#pspV124Schedule'));
    });
    if(!external)return;
    clearTimeout(window.__v124CourseT);window.__v124CourseT=setTimeout(renderSchedule,90);
