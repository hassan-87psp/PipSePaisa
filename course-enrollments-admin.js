...
function realtime(){const client=db();if(!client||window.__aceV20Realtime)return;window.__aceV20Realtime=true;try{client.channel('psp-course-enrollments-admin-v20').on('postgres_changes',{event:'*',schema:'public',table:'course_enrollments'},()=>window.loadAdminCourseEnrollments()).subscribe()}catch(e){console.warn(e)}}
function init(){
  inject();wrap();realtime();setTimeout(window.loadAdminCourseEnrollments,500);
