(function(){
  'use strict';
  if(window.__pspLinkManagerV211)return;
  window.__pspLinkManagerV211=true;
  ...
  function conversionRate(enrollments,uniqueVisitors){const u=Number(uniqueVisitors||0),e=Number(enrollments||0);return u>0?Math.min(100,(e/u)*100):0;}
  ...
  function addMenuAndPage(){
    ...
    page.innerHTML=`
      ...
      <div class="card">
        <div class="card-header"><div><div class="card-title">Tracked Links Performance</div><div class="card-meta">Clicks, unique visitors and conversions update automatically. Conversion = enrolled users ÷ unique visitors.</div></div><button class="btn btn-secondary btn-sm" id="lmRefreshBtn">↻ Refresh</button></div>
        <div class="table-wrap"><table class="data-table"><thead><tr><th>Link</th><th>Source</th><th>Destination</th><th>Clicks</th><th>Unique</th><th>Signups</th><th>Enrolled Users</th><th>Conversion</th><th>Status</th><th>Actions</th></tr></thead><tbody id="lmTable"><tr><td colspan="10">Open Link Manager to load data.</td></tr></tbody></table></div>
      </div>
  ...
  async function loadTeamMembersV203(){
    ...
    sel.disabled=true;
    sel.innerHTML='<option value="">Loading team members…</option>';
    ...
    try{
      ...
      toast('Team members could not be loaded. Please refresh Link Manager.','err');
  ...
  async function createLink(){
    ...
    try{
      ...
      const teamSel=document.getElementById('lmTeamMember');
  ...
  async function loadLinks(){
    ...
    tbody.innerHTML=statsRows.map(x=>`<tr>
      ...
      <td>${esc(x.source||'—')}<div style="font-size:9px;color:var(--text-muted);margin-top:4px">${x.whatsapp_number?'WhatsApp: '+esc(x.whatsapp_number):'Channel fallback'}</div></td><td>${esc(x.destination_path)}</td><td><strong>${fmt(x.total_clicks)}</strong></td><td>${fmt(x.unique_visitors)}</td><td>${fmt(x.signups)}</td><td>${fmt(x.enrollments)}</td><td>${conversionRate(x.enrollments,x.unique_visitors).toFixed(1)}%</td>
